...
function handleYesClick() {
    // Start background audio (user gesture) then replace the question and buttons
    startYTBackground();
    ...
    replaceQuestionWithMessage("I love you Olwethu");
...
// --- YouTube background audio helper ---
var _ytStarted = false;
var _ytPlayer = null;
function startYTBackground() {
    if (_ytStarted) {
        try { _ytPlayer && _ytPlayer.playVideo && _ytPlayer.playVideo(); } catch (err) {}
        return;
    }
    _ytStarted = true;
    _insertYTPlayerMarkup();
...
function _insertYTPlayerMarkup(){
    var existing = document.getElementById('yt-player-container');
    if (existing) return;
